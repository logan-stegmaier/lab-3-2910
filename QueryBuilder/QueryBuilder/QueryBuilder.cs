...
namespace QueryBuilder
{
    public class QueryBuilder : IDisposable
    {
        ...
        {
            PropertyInfo[] properties = typeof(T).GetProperties(); // grab property values

            var values = new List<string>(); //creating the list to store values
            var names = new List<string>(); // creating list to store the names
            ...
            for (int i = 1; i < properties.Length; i++)
            {
                ...
                else if (property.PropertyType == typeof(string)) // formatting the string for the database
                {
                    values.Add($"\"{property.GetValue(obj).ToString()}\"");
                }
                else
                {
                    values.Add(property.GetValue(obj).ToString()); //formatting other types for DB
                }
                names.Add(property.Name);
            }

            StringBuilder sb = new StringBuilder(); // beginning build for insert statement
            StringBuilder sb2 = new StringBuilder(); // second string builder for actual values 

            for (int i = 0; i < values.Count; i++) // include commas in between values / names UNLESS you're on the last item
            {
                if (i == values.Count - 1)
                {
                    sb.Append($"{names[i]}");
        ...
        // write a DeleteALL method that deletes items from given table 


        public void DeleteAll<T>(T obj) where T : IClassModel
        {
            var command = connection.CreateCommand();
            command.CommandText = $"DELETE FROM {typeof(T).Name}"; // deletes everything from table name 
            var reader = command.ExecuteNonQuery(); // executes
